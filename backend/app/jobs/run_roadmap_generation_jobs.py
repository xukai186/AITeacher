"""Run pending roadmap generation jobs.

Example crontab (every minute):
  * * * * * cd /path/to/AITeacher/backend && .venv/bin/python -m app.jobs.run_roadmap_generation_jobs --once
"""
...
def main() -> int:
    ...
    runner = RoadmapGenerationJobRunner()
    while True:
        db = SessionLocal()
        try:
